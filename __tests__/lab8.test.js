describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 1; i < products.length; i++){
      console.log(`Checking product item ${i + 1}/${products.length}`);
      product = await products[i].getProperty('shadowRoot');
    ...
    const cartCountData = await cartCount.getProperty("innerText");
    const value = await cartCountData["_remoteObject"].value;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 1; i < products.length; i++){
      ...
      const buttonText = await buttonInnerText["_remoteObject"].value;
